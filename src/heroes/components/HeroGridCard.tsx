    ...
    return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-to-br from-white to-gray-50">
            <div className="relative h-64">
                <img
                    src={`${hero.image}?height=300&width=300`}
                    alt="Superman"
            ...
            <div className="absolute top-3 left-3 flex items-center gap-2">
                ...
                {
                    (hero.universe === 'DC') ? (
                        <Badge className="absolute top-3 right-3 text-xs bg-blue-600 text-white font-bold">{hero.universe}</Badge>
                    ) : (
                        <Badge className="absolute top-3 right-3 text-xs bg-red-600 text-white font-bold">{hero.universe}</Badge>
                    )
                }
                ...
                > 
                    <Heart className={`h-4 w-4 ${
                        isFavorite(hero) ? 'text-red-500 fill-red-500' : 'text-gray-500'
                    }`} />
                </Button>
